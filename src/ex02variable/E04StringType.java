...
public class E04StringType {
	...
	public static void main(String[] args) {
		...
		/*
		 string 타입 : 참조형 변수로써 기본 자료형이 아님, 문자열을 저장할 수 있는 자료형으로 실제로는 class임.
		 			  자바에서는 문자열을 표현할 때 " 으로 감싸면 되고, 연결할 때 + 기호를 사용한다.
		 */
		int number;
		number = 99;
		/*
		 참조형 변수와 기본 자료형 변수 사이에서는 형변환이 불가능하다. 서로 사용하는 메모리 공간이 다르기 때문이다.
		...
		/*
		 문자열 + 정수 => 문자열 | 즉, 문자열에 정수가 연결된 형태로 출력된다.
		 */
		...
		/*
		 문자열+숫자 => 우선 순위가 없다면 일반적으로 문자열로 출력된다. 단, 산술연산의 결과를 출력하고
		 싶다면 아래와 같이 소괄호를 이용해서 우선 순위에 대한 변화를 주면 된다.
		 */
