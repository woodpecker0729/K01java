...
//		}
		...
		// 조건검사 후 실행할 문장이 하나인 경우에는 중괄호를 생략할수 있따. 단, 실행문이 2개 이상인 경우에는 한문장만 실행되므로 주의해야 한다.
		if (num % 2 == 0) {
			System.out.println("\n" + num + "은 짝수");
		}
		if (num % 2 != 0) {
		...
		/*
		 System.in.read() : 사용자에게 하나의 문자를 입력받을때 사용하는 함수(메소드)로 입력한 문자의 아스키코드값을
		 반환한다. 해당 함수는 IOException을 기본적으로 가지고 있어서 이를 무시하기 위해 main()에서 throw 처리를 해준다.
		 */
...
//		System.out.println("입력한문자:"+(char)asciicode);
		...
		System.out.print("하나의 문자를 입력하세요:");
		int asciicode = System.in.read();
		if(asciicode>=48&&asciicode<=57) System.out.println("입력한 문자는 숫자입니다.");
		else System.out.println("입력한 문자는 숫자가 아닙니다.");
//		
	}
